...
def main():
    ...
    start = str(INPUT('Select W or F to launch wireshark or save a sniff in file pcap: '))
    if start == 'f' or start == 'F':
        name = str(INPUT('Digit a name: '))
        name_replace = name.replace(' ', '-')
        name_replace = name.replace('.pcap', '')
        if name != name_replace:
            print "There was an error with", name
            print name, "has been renamed to", name_replace
            args.pcap_file = name_replace + '.pcap'
        else:
            args.pcap_file = name + '.pcap'
        path_file = str(os.path.abspath(args.pcap_file))
